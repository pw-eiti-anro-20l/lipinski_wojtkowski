...
SubscribePublish::SubscribePublish() {
	...
	double params[6];
	_n.getParam("a1", params[0]);
	_n.getParam("a2", params[1]);
	_n.getParam("alpha2", params[2]);
	_n.getParam("d3", params[3]);
	_n.getParam("theta1", params[4]);
	_n.getParam("theta2", param[5]);
	_robot = Robot(params);
}
...
void SubscribePublish::_publish(const ros::Publisher& pub, const KDL::Frame& pos) {
	...
	msg.header.frame_id = pu.getTopic();
	pub.publish(msg);
}