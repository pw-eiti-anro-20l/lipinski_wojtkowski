...
jlimit = [rospy.get_param('~joint1_lim')*pi, rospy.get_param('~joint2_lim')*pi, rospy.get_param('~joint3_lim')]
=======
a1 = rospy.get_param('/a1')
a2 = rospy.get_param('/a2')
d3 = rospy.get_param('/d3')
...
theta1 = rospy.get_param('/theta1')
theta2 = rospy.get_param('/theta2')
jlimit = [rospy.get_param('/joint1_lim')*pi, rospy.get_param('/joint2_lim')*pi, d3*rospy.get_param('/joint3_lim')]
>>>>>>> MTL
joints=[]
...
<<<<<<< HEAD
	...
	T = None
	for joint in joints:
		if abs(pos_change[i])>jlimit[i]:
			rospy.loginfo('[nonkdl] joint%d is out of bounds (|%f| > %f)', i+1, pos_change[i], jlimit[i])
			i=i+1
		else: 
			if joint.rot:
