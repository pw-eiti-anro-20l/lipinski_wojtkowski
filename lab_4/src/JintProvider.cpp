...
bool JintProvider::callback(lab_4::jint_control_srv::Request& req, lab4::jint_control_srv::Response& resp) {
	...
	if(int_type == "lin")
		...
		resp->status=false;
		ROS_ERROR("Interpolation \"" + int_type "\" not supportd"); 
		return false;
	}
	
	if(mov_duration.toSec() <= 0) {
		resp->status=false;
		ROS_ERROR("Duration less or equal 0")
		return false;
	}
	...
	for(int i = 0; i < 3; ++i) {
		...
		case Interpolation::LIN:
			...
			params.push_back(_position[i] - v * start);
			params.push_back(v);
			break;
		case Interpolation::CUBE:
			calc_cube_params(final_joint_states[i], _position[i], mov_duration.toSec(), start.toSec(), vect);
			break;
		}
		interpol_params.append(params);
	}
